...
const Userchat = ({item, userId}) => {
  ...
  return (
    <Pressable onPress={() => router.push({
        ...
        <View>
            <Image source={{uri: item?.profileImages[0]}} style={{width: 60, height: 60, borderRadius: 35}} />
        </View>

        <View>
            <Text style={{fontSize: 15, fontWeight: '500', marginTop: 6, color: 'white'}}>{item?.name}</Text>
            <Text style={{color: 'white'}}>{lastMessage ? lastMessage?.message : `Start the chat with ${item?.name}`}</Text>
        </View>
