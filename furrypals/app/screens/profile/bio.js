...
import {Text, ScrollView, View, Pressable, Image, TextInput, Button, Alert, FlatList, KeyboardAvoidingView} from 'react-native'
...
const bio = () => {
  ...
  const updateUserDescription = async () => {
    try {
      const response = await axios.put(`http://10.0.0.11:5000/users/${userId}/description`, {description: description})
  ...
  const handelAddImage = async () => {
    try {
      const response = await axios.post(`http://10.0.0.11:5000/users/${userId}/profile-images`, {imageUrl: imageUrl})
  ...
  const renderImageCarousel = ({item}) => (
    <View style={{ width: "100%", justifyContent: "center", alignItems: "center" }}>
      <Image source={ images} style={{resizeMode: 'cover', height: 200, borderRadius: 10, width: '85%', transform: [{ rotate: "-5deg" }]}} />
      <Text  style={{ position: "absolute", top: 10, right: 2, color: "black" }}>{activeSlide + 1}/{images.length}</Text>
    </View>
  )
  ...
  const addLookingFor = async (lookingFor) => {
    try {
      const response = await axios.put(`http://10.0.0.11:5000/users/${userId}/looking-for`, {lookingFor: lookingFor})
      if(response.status == 200) {
  ...
  const removeLookingFor = async (lookingFor) => {
    try {
      const response = await axios.put(`http://10.0.0.11:5000/users/${userId}/looking-for/remove`, {lookingFor: lookingFor})
      if(response.status == 200) {
  ...
  return (
    <ScrollView style={{backgroundColor: 'green', marginTop: 3}}>
      <Text style={{fontSize: 32, fontWeight: 'bold', marginTop: 4, marginLeft: 4, color:'#452817' }}>FurryPals</Text>
      <View >
        <View>
          <View>
            <Pressable style={{
              ...
              <Image style={{width: 50, height: 60, borderRadius: 20}} source={{uri: 'https://images.unsplash.com/photo-1552053831-71594a27632d?q=80&w=1562&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D'}} />
              <Text  style={{ fontSize: 16, fontWeight: "600", marginTop: 6, color: 'white' }}>Buster</Text>
              <Text  style={{ fontSize: 16, fontWeight: "600", marginTop: 6, color: 'white' }}>I am 2 years old</Text>
            </Pressable>
          </View>
        </View>
      </View>

      <View style={{ marginTop: 180, marginHorizontal: 20, flexDirection: "row", alignItems: "center", gap: 25, justifyContent: "center",}}>
        <Pressable onPress={() => setOption('about')}>
          <Text style={{color: option == 'about' ? 'white' : 'black'}}>About</Text>
        </Pressable>
        <Pressable onPress={() => setOption('photos')}>
          <Text style={{color: option == 'photos' ? 'white' : 'black'}}>Photos</Text>
        </Pressable>
        <Pressable onPress={() => setOption('looking for')}>
          <Text style={{color: option == 'looking for' ? 'white' : 'black'}}>Looking for</Text>
        </Pressable>
      </View>
        ...
        {option == 'photos' && (
          <View >
            ...
            <View style={{ marginTop: 2 }}>
              <Text style={{fontSize: 20, color: '#452817', margin: 4}}>Add photos of pup</Text>
              <View style={{
                  flexDirection: "row",
                  alignItems: "center",
                  gap: 5,
              ...
              style={{marginTop: 5}} title='Add image'></Button>
            </View>
          </View>
        )}
      </View>
          ...
          <>
            <View>
              ...
              renderItem={({item}) => (
                ...
                style={{
                  backgroundColor: lookingOptions.includes(item?.name) ? '#FDBB2D' : '#452817',
                  padding: 16,
                  justifyContent: 'center',
                  alignItems: 'center', 
                  width: 150,
                  ...
                  borderRadius: 5,
                  borderColor: 'white', 
                  borderWidth: lookingOptions.includes(item?.name || '') ? 0 : 2
                }}
                  >
                  <Text style={{
                        textAlign: "center",
                        fontWeight: "500",
                        fontSize: 13,
                        color: lookingOptions.includes(item?.name) ? "#452817" : "white",
                        }}
                      >{item.name}</Text>
                  <Text style={{
                        color: lookingOptions.includes(item?.name) ? "#452817" : "white",
