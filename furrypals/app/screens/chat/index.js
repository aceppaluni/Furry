...
const index = () => {
  ...
  const fetchRecievedLikesDetails = async () => {
    try {
      const response = await axios.get(`http://10.0.0.11:5000/received-likes/${userId}/details`)
  ...
  const fetchUserMatches = async () => {
    try {
      const response = await axios.get(`http://10.0.0.11:5000/users/${userId}/matches`)
