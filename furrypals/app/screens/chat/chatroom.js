    ...
    navigation.setOptions({
      ...
      headerLeft: () => (
        <View style={{flexDirection: 'row', alignItems: 'center', gap: 10}}>
          ...
          <View style={{flexDirection: 'row', alignItems: 'center', gap: 10}}>
            <Image source={{uri: params?.image}} style={{width: 30, height: 30, borderRadius: 15, resizeMode: 'cover'}} />
    ...
    <KeyboardAvoidingView style={{flex: 1, backgroundColor: 'white'}}>
      <ScrollView contentContainerStyle={{flexGrow: 1}}>
        {messages.map((item, index) => (
          <Pressable style={[item?.senderId === params?.senderId ? {
            alignSelf: 'flex-end',
      ...
      <View style={{
        ...
        <TextInput value={message} onChangeText={(text) => setMessage(text)} style={{
            flex: 1,
            height: 40,
            borderWidth: 1,
            borderColor: "#dddddd",
        ...
        <Pressable onPress={() => sendMessage(params?.senderId, params?.receiverId)} style={{
